...
export default function MultipleImagesUploader({onChange}: {onChange: Dispatch<React.SetStateAction<[] | (File | FileMetadata)[]>>}) {
  const maxSizeMB = 5
  ...
  useEffect(()=>{
    if(files.length > 0){
      const imagesList = files.map((item)=> item.file);
      onChange(imagesList);
